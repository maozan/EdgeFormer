...
from models.EdgeDetection import Edge
...
class CDDataset_edge(ImageDataset):

    def __init__(self, root_dir, img_size, split='train', is_train=True, label_transform=None,
                 to_tensor=True):
        super(CDDataset_edge, self).__init__(root_dir, img_size=img_size, split=split, is_train=is_train,
                                        to_tensor=to_tensor)
    ...
    def __getitem__(self, index):
        ...
        Roberts, Prewitt, Sobel, Laplacian = Edge(img)
        # i = Image.fromarray(img)
        # i.save('./img.jpg')
        # r = Image.fromarray(Roberts)
        # r.save('./r.jpg')
        # p = Image.fromarray(Prewitt)
        # p.save('./p.jpg')
        # s = Image.fromarray(Sobel)
        # s.save('./s.jpg')
        # l = Image.fromarray(Laplacian)
        # l.save('./l.jpg')
        # print('=======')
        # exit()
        Roberts_B, Prewitt_B, Sobel_B, Laplacian_B = Edge(img_B)
        
        [img, img_B, Roberts, Prewitt, Sobel, Laplacian, Roberts_B, Prewitt_B, Sobel_B, Laplacian_B], [label] = self.augm.transform([img, img_B, Roberts, Prewitt, Sobel, Laplacian, Roberts_B, Prewitt_B, Sobel_B, Laplacian_B], [label], to_tensor=self.to_tensor)
        # print(label.max())

        img_edge = torch.cat((Roberts, Prewitt, Sobel, Laplacian), dim=0)
        img_B_edge = torch.cat((Roberts_B, Prewitt_B, Sobel_B, Laplacian_B), dim=0)
        
        return {'name': name, 'A': img, 'B': img_B, 'L': label, 'A_edge': img_edge, 'B_edge': img_B_edge}
